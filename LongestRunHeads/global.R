...
flip.gen <- function(trials,prob){
  ...
  coords <- matrix(NA,ncol=4,nrow=(length(coins)))
  
  colnames(coords) <- c("coin","segment","run","head_head")
  
  coords[,1]<-coins
  coords[,4]<-rep(0, length(coins))
  ...
  for (i in 2:length(coins)){
    ...
    if (coin1 == 1 & coin2 == 1) coords[i, 4] <- 1
  }
...
plot.flips <- function(run.min,coords,my.cex,sub.flag){
  ...
  my.title <- paste("n = ", nrow(coords), ", Longest run = ",max.run,
                    ", P(H|H): ", round(sum(coords[,4]) / sum(coords[,1][1:nrow(coords)-1]), 4),
  ...
  for (i in 1:nrow(coords)){
    my.coin <- coords[i,1]
    if (my.coin==1) {
      if (coords[i,3]>=run.min) text(coords[i,5],coords[i,6],"H",
                                     col="red",font=2,cex=my.cex)
      else text(coords[i,5],coords[i,6],"H",cex=my.cex)}
    if (my.coin==0) {
      if (coords[i,3]>=run.min) text(coords[i,5],coords[i,6],"T",
                                     col="deepskyblue",font=2,cex=my.cex)
      else text(coords[i,5],coords[i,6],"T",cex=my.cex)}
  }
}